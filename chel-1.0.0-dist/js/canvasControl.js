...
CanvasControl.prototype.initKeyboardEvents = function (self)
{
  ...
  CanvasControl.prototype.render = function () 
  {
    ...
    if (self._value != '')
    {
      ...
      self._ctx.fillStyle = self._fontColor;

      var dy =  (self._height / 2) + self.textHeight(self._ctx, self._value  ) / 4;
      ...
      if (self._controlType == self.C_CONTROL_TYPE_INPUT)
      {
        self.renderTextLeft(self._ctx , self._x, self._y + dy, self._value);
      }
      else
      {
        ...
        if (self._textJustify == 0)
          self.renderTextCentered(self._ctx , self._x + dx, self._y + dy, self._value);
  ...
  */
  ...
  CanvasControl.prototype.msglog = function(_text) 
  {
    ...
    if (_themeType == CanvasControl.C_THEME_TYPE_GREEN)
    {
      ...
      this._borderColor = "rgb(214,233,198)";

      if (this._controlType == this.C_CONTROL_TYPE_LABEL)
        this._fontColor = 'rgb(166,109,59)';
    }
